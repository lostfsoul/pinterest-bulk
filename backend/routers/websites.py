...
def _deep_merge_dicts(base: dict, override: dict | None) -> dict:
    """Merge saved website settings over defaults without dropping nested keys."""
    result = dict(base)
...
def _default_generation_settings(*, onboarding_required: bool = False) -> dict:
    settings = {
        ...
        "onboarding": {
            ...
            "completed": not onboarding_required,
        },
...
def _website_generation_settings(website: Website) -> dict:
    defaults = _default_generation_settings(onboarding_required=False)
    return _deep_merge_dicts(defaults, website.generation_settings)
    ...
    for site in websites:
        ...
        result.append(
            WebsiteWithStats(
                ...
                generation_settings=site.generation_settings,
...
def get_website(website_id: int, db: Session = Depends(get_db)):
    ...
    return WebsiteWithStats(
        ...
        generation_settings=website.generation_settings,
...
def get_generation_settings(website_id: int, db: Session = Depends(get_db)):
    ...
    return WebsiteGenerationSettingsResponse(
        ...
        settings=_website_generation_settings(website),
    )
...
def update_generation_settings(
    ...
    website.generation_settings = _deep_merge_dicts(_website_generation_settings(website), update.settings)
    db.commit()
    db.refresh(website)
    return WebsiteGenerationSettingsResponse(
        website_id=website.id,
